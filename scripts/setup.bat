...
docker run --rm -v "%cd%\data\certs:/certs" alpine/openssl req -x509 -nodes -days 365 -newkey rsa:2048 ^
...
docker-compose exec -T minio sh -c "^
    ...
    mc mb -p myminio/compliance-data && ^
    mc mb -p myminio/compliance-reports && ^
